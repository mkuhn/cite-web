    ...
    * Generates the appropriate redirect based on the OpenID protocol
    ...
    if request.POST:
        ...
        if error:
            ...
            return renderIndexPage(request, url_hash, error=error)
    ...
    return renderIndexPage(request, url_hash)

def finishOpenID(request, url_hash):
    """
    ...
    if request_args:
        ...
        if response.status == consumer.SUCCESS:
            ...
            userprefs_l = models.UserPrefs.objects.filter(user = user)
            
            if url_hash == "view":
                if userprefs_l:
                    userprefs = userprefs_l[0]

                    return HttpResponseRedirect("/view/stable/%s" % userprefs.user_hash)
            else:
    ...
    return renderIndexPage(request, url_hash, **result)

def rpXRDS(request, url_hash):
    """
